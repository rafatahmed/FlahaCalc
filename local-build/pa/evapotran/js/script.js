...
function loadSavedData() {
    try {
        const savedData = localStorage.getItem('etoCalcData');
        if (savedData) {
            const data = JSON.parse(savedData);
            console.log('Loading saved calculator data:', data);
            ...
            // If elevation is estimated, show a notification
            if (data.elevationEstimated) {
                const elevationField = document.getElementById('elevation');
                if (elevationField) {
                    // Add a warning class to the field
                    elevationField.classList.add('estimated-value');
                    
                    // Add a notification
                    const formGroup = elevationField.closest('.form-group');
                    if (formGroup) {
                        const notification = document.createElement('div');
                        notification.className = 'field-notification';
                        notification.innerHTML = 'Estimated value. Please verify or update.';
